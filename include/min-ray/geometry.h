...
class Vector2 {
 public:
  ...
  Vector2() { x = y = 0; }
  Vector2(T xx, T yy) : x(xx), y(yy) { MIN_ASSERT(!HasNaNs()); }
...
class Vector3 {
 public:
  ...
  Vector3() { x = y = z = 0; }
  Vector3(T x, T y, T z) : x(x), y(y), z(z) { MIN_ASSERT(!HasNaNs()); }
...
class Point2 {
 public:
  ...
  explicit Point2(const Point3<T> &p) : x(p.x), y(p.y) { MIN_ASSERT(!HasNaNs()); }
  Point2() { x = y = 0; }
  Point2(T xx, T yy) : x(xx), y(yy) { MIN_ASSERT(!HasNaNs()); }
...
class Point3 {
 public:
  ...
  Point3() { x = y = z = 0; }
  Point3(T x, T y, T z) : x(x), y(y), z(z) { MIN_ASSERT(!HasNaNs()); }
  template <typename U>
...
class Normal3 {
 public:
  ...
  Normal3() { x = y = z = 0; }
  Normal3(T xx, T yy, T zz) : x(xx), y(yy), z(zz) { MIN_ASSERT(!HasNaNs()); }
  Normal3<T> operator-() const { return Normal3(-x, -y, -z); }
  Normal3<T> operator+(const Normal3<T> &n) const {
    MIN_ASSERT(!n.HasNaNs());
...
class Bounds2 {
 public:
  ...
  std::string ToString() const {
    return fmt::format("[ {} - {} ]", pMin.ToString(), pMax.ToString());
  }
...
class Ray {
 public:
  // Ray Public Methods
  Ray() : tMax(kInfinity), time(0.f) {}
  Ray(const Point3f &o, const Vector3f &d, Float tMax = kInfinity,
      Float time = 0.f)
...
class RayDifferential : public Ray {
 public:
  ...
  RayDifferential(const Point3f &o, const Vector3f &d, Float tMax = kInfinity,
                  Float time = 0.f)
      : Ray(o, d, tMax, time) {
...
inline T Dot(const Vector3<T> &v1, const Vector3<T> &v2) {
  MIN_ASSERT(!v1.HasNaNs() && !v2.HasNaNs());
...
Point2<T> Min(const Point2<T> &pa, const Point2<T> &pb) {
  return Point2<T>(std::min<T>(pa.x, pb.x), std::min<T>(pa.y, pb.y));
}

template <typename T>
Point2<T> Max(const Point2<T> &pa, const Point2<T> &pb) {
  return Point2<T>(std::max<T>(pa.x, pb.x), std::max<T>(pa.y, pb.y));
}
...
inline Float DistanceSquared(const Point3<T> &p, const Bounds3<U> &b) {
  Float dx = std::max<Float>({Float(0), b.pMin.x - p.x, p.x - b.pMax.x});
  Float dy = std::max<Float>({Float(0), b.pMin.y - p.y, p.y - b.pMax.y});
  Float dz = std::max<Float>({Float(0), b.pMin.z - p.z, p.z - b.pMax.z});
