...
class CoefficientSpectrum {
 public:
  ...
  friend inline CoefficientSpectrum operator*(Float a,
    ...
    MIN_ASSERT(!std::isnan(a) && !s.HasNaNs());
    return s * a;
  }
  CoefficientSpectrum operator/(Float a) const {
    CHECK_NE(a, 0);
    MIN_ASSERT(!std::isnan(a));
    CoefficientSpectrum ret = *this;
    for (int i = 0; i < nSpectrumSamples; ++i) ret.c[i] /= a;
    MIN_ASSERT(!ret.HasNaNs());
    return ret;
  ...
  CoefficientSpectrum Clamp(Float low = 0, Float high = kInfinity) const {
    CoefficientSpectrum ret;
    for (int i = 0; i < nSpectrumSamples; ++i)
      ret.c[i] = min::ray::Clamp(c[i], low, high);
    MIN_ASSERT(!ret.HasNaNs());
    return ret;
  }
  ...
  Float &operator[](int i) {
    MIN_ASSERT(i >= 0 && i < nSpectrumSamples);
    return c[i];
  }
