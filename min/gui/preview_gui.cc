...
void PreviewGUI::Init() {
  ...
  const Vector2i size = film->full_resolution;
...
void PreviewGUI::InitDrawcalls() {
  ...
  glVertexAttribPointer(attrib_id, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), 0);
  // texture
...
#define TO_BYTE(v) (uint8_t) Clamp(255.f * GammaCorrect(v), 0.f, 255.f)
      dst[0] = TO_BYTE(spectrums[y * size.x + x].x);
      dst[1] = TO_BYTE(spectrums[y * size.x + x].y);
      dst[2] = TO_BYTE(spectrums[y * size.x + x].z);
#undef TO_BYTE
      dst += 3;
    }
  }
  ...
  glBindTexture(GL_TEXTURE_2D, texture);
  glPixelStorei(GL_UNPACK_ROW_LENGTH, (GLint)size.x);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, size.x, size.y,
               0, GL_RGB, GL_UNSIGNED_BYTE, rgb8.get());
